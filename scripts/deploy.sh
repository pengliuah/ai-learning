...
#   - 将阿里云下载的 SSL 证书放到 /opt/zhixue/nginx/ssl/zhixue.pem 和 zhixue.key
#
set -euo pipefail
...
if [ ! -f "$SSL_DIR/zhixue.pem" ] || [ ! -f "$SSL_DIR/zhixue.key" ]; then
  warn "SSL 证书缺失: $SSL_DIR/zhixue.pem 或 zhixue.key"
  warn "从阿里云 SSL 证书控制台下载 (Nginx 格式) 后放到 $SSL_DIR/"
  warn "nginx 容器将无法启动 HTTPS; 请放置证书后重新运行本脚本"
fi
