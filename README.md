...
系统支持把学习成果一键存入腾讯 [ima](https://ima.qq.com) 知识库，方便在微信生态里随时检索回顾：

- **可保存的内容**：计划总览、模块学习内容（含关键要点）、测验题目、批改结果与逐题解析；AI 教练对话中说出"存档"也会弹出保存卡片；
- **配置方法**：登录后进入「设置 → 保存到 IMA 设置」，前往 [ima.qq.com/agent-interface](https://ima.qq.com/agent-interface) 获取 `IMA_OPENAPI_CLIENTID` 与 `IMA_OPENAPI_APIKEY` 填入即可；
- **Skill 策略**：用自然语言描述保存策略（例如保存范围、格式要求、目标笔记本），保存时会先由大模型按该策略整理内容，再写入你的 IMA 知识库；
- 未配置 IMA 不影响其他任何功能，配置后计划页的「保存到 IMA」按钮即可使用。
